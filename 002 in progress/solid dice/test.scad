...
module top_half() {
  ...
  difference() {
    union() {
      ...
      //ribs
      translate([0,0,.2])mirror2([1, 1, 0]) linear_extrude(2*r*cut_height-.2) square([gap+2*extrusion_width, box_width], center = true);
    }
    //cup
    translate([0, 0, r]) b();
  ...
  difference() {
    ...
    mirror2([1,1,0]) linear_extrude(2*r*cut_height) square([box_width, gap], center=true);
    //center hole
