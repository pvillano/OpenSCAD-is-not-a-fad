...
module base() {
  ...
  h = plate_stackup + usb_pcb_thicc + .2;
  difference() {

//    translate([-wall_thickness, -wall_thickness + pcb_ys[0], -h + plate_stackup])
//      cube([pcb_xs[4] + 2 * wall_thickness, pcb_ys[2] - pcb_ys[0] + 2 * wall_thickness, h]);

    hull() {
      translate([-wall_thickness, pcb_ys[2] + wall_thickness, -h + plate_stackup])
        cube([pcb_xs[4] + 2 * wall_thickness, ep, h]);
      translate([-wall_thickness, -wall_thickness + pcb_ys[0], -.2])
        cube([pcb_xs[4] + 2 * wall_thickness, ep, plate_stackup + .2]);
    }
    translate([0, pcb_ys[0], 0])
      cube([pcb_xs[4], pcb_ys[2] - pcb_ys[0], plate_stackup + ep]);
    ...
    for (i = [0:8], j = [0:5]) {
      ...
      translate([x, y, 0]) {
        translate([0, 0, -ep])
          cylinder(d = 5, h = 999, center=true); // center stud
      }
    }
  }
}
