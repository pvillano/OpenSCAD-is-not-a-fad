...
module hyperboloid3(h, r0, r1, thickness, num_legs, percent=.25){
	...
	function rule_line_point (alpha,t,a,b,c,h) = [a*cos(alpha),b*sin(alpha),0] +
		t*(h/c)*[-a*sin(alpha),b*cos(alpha),c];
	...
	ok so I want the 
	*/
	z_count=max(3,ceil(h*sqrt(1+unslope^2)/$fs));
	r_count = ceil(2*PI*r1/$fs/num_legs*percent);
	alpha_range = 360/num_legs*percent;
	
	points = [ for(t=[0:z_count-1],alpha=[0:r_count-1]) 
		rule_line_point(alpha/(r_count-1)*alpha_range,t/z_count,a,b,c,h)
	];
	points2 = [for (p=points) each [p, inset_xy(p, thickness)]];
	
	//each point is assigned a face except top points
	faces = [for(t=[0:z_count-2],alpha=[0:r_count-1])
		[
			t*r_count+alpha,
			r_count+((alpha+1)%r_count),
			(t+1)*r_count+alpha,
			(t+1)*r_count++((alpha+1)%r_count)
		]
	];
	polyhedron(points,faces);
	//for(p=points) translate(p) cube(3);
	cube();
}
